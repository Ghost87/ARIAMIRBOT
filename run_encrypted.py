...
def main() -> int:
    ...
    try:
        ...
        # 🔗 پوشهٔ data باید به بیرون اشاره کند تا دیتابیس بعد از پاک‌شدن
        # پوشهٔ موقت باقی بماند. اگر آرشیو خودش data/ داشت (خالی یا با فایل)،
        # اول کنارش می‌گذاریم و بعد لینک می‌سازیم — وگرنه FileExistsError.
        inner = src / "data"
        if inner.is_symlink() or inner.is_file():
            inner.unlink()
        elif inner.is_dir():
            for item in inner.iterdir():          # چیز باارزشی داخلش بود؟ بیرون ببر
                target = data_dir / item.name
                if not target.exists():
                    shutil.move(str(item), str(target))
            shutil.rmtree(inner, ignore_errors=True)
        os.symlink(str(data_dir), str(inner))
